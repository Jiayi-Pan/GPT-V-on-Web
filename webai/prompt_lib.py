...
Link Hint mode:
Once you find an element you want to click, you can press "f" in Normal mode to enter Link Hint mode. Yellow labels from "A" to "ZZ" will appear on-screen and typing the label triggers a click on that element. If the element is an input, you'll enter Type mode. **You are in Link Hint mode if you see a lot of yellow labels on the screen.**
...
- You are directly brought into Type mode when you are at google.com's start page. So you can just type your query and press <ENTER> to search.
...
def get_context_prompt(last_response, memory, chat_history):
    ...
    mem_str = '\n'.join(memory)
    return f"Chat History: {chat_str}.\n Your last move: {last_response}\n Info you wrote in memory: {mem_str}\n {'You are in Link Hint mode.' if last_response['next_action'] == 'f' else 'You are not in Link Hint mode.'}"